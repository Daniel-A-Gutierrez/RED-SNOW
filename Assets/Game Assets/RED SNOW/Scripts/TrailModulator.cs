...
public class TrailModulator : MonoBehaviour {
	public double stopHeight = 2;
	public double appearSpeed = 7f;
	...
	void LateUpdate () {
		...
		if(hit.collider == null || hit.distance > stopHeight || playerRb.velocity.magnitude < appearSpeed)
			em.enabled = false;
		else if(!em.enabled)
			em.enabled = true;
	}
